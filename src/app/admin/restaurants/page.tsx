// src/app/admin/restaurants/page.tsx - FIXED VERSION
'use client';
              ...
              <Card key={restaurant._id}>
                ...
                <div className="aspect-video bg-gradient-to-br from-gray-200 to-gray-300 relative">
                  <div 
                    ...
                    role="img"
                    aria-label={`${restaurant.name} icon`}
                  >
                    {restaurant.image || '🍽️'}
                  </div>
                  {!restaurant.isOpen && (
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
                        <Star className="w-4 h-4 text-yellow-500" fill="currentColor" aria-hidden="true" />
                        <span className="font-semibold" aria-label={`Rating ${restaurant.rating} out of 5`}>
                          {restaurant.rating}
                        </span>
                      </div>
                    </div>
                  ...
                  {restaurant.cuisine?.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-4" role="list" aria-label="Cuisine types">
                  ...
                  {/* ACTION BUTTONS */}
                  <div className="grid grid-cols-3 gap-2" role="group" aria-label="Restaurant actions">
                    <button
                      onClick={() => toggleRestaurantStatus(restaurant._id, restaurant.isOpen)}
                      aria-label={restaurant.isOpen ? 'Hide restaurant from customers' : 'Show restaurant to customers'}
                      title={restaurant.isOpen ? 'Hide restaurant' : 'Show restaurant'}
